...
You can customize your own pattern to a suitable size using our [online pattern generator](https://techlauncher-leafanalyzer.github.io/Leaf-Analyzer-Pattern-Generation/). Please make sure to print the generated PDF file at 100% scale for accuracy – you can check the pattern dimensions by comparing the actual measurements with a ruler and the numbers written on the top right corner of the PDF file. At the top of each PDF (to the right of the logo), you’ll see labels like 120×120–15 mm, which follow the format *pattern width* × *pattern height* – *AprilTag side length*. The [Patterns](Patterns/) folder contains some older version PDF pattern files (A4–A1). If you use those older version pattern files, you'll need to manually enter the pattern parameters in Settings → Pattern (Fig. 6b). If your pattern is generated by our online pattern generator, the pattern parameters should be automatically read by the software.
...
The proposed LBS feature in the paper works well for leaf segmentation in most cases. However, in some challenging situations, more features may need to be added to improve the segmentation. For example, when leaves have strong reflection, you may need to incorporate texture features; when dark leaves are imaged under uneven lighting, you can add the Dark Green Color Index (DGCI) feature.
...
### 2.5 Switch to Thresholding method for segmentation
